...
fn calc_image(buffer: &mut Vec<u32>, grid:&mut GridObject, loop_counter: u32) {
    clear_buffer(buffer);
    draw_grid(buffer, grid, loop_counter);
    grid.apply_rules();

}

fn draw_grid(buffer: &mut Vec<u32>, grid:&mut GridObject, loop_counter: u32){
                ...
                true => {
                    for y_off in 0..cell_size{
                        for x_off in 0..cell_size{
                            draw_pixel(buffer, (x*cell_size + x_off) as u32, (y*cell_size + y_off) as u32, _YELLOW);
                        }
                    }
                },
                false => ()
